...
struct
	...
	and unify' [] = []
      | unify' ((tyS,UVar x) :: rest) =  
		if ty_eq tyS (UVar x) then (print "unify1\n";unify rest)
     ...
     | unify' ((VectorTy t1, VectorTy t2) :: rest) = 
	 	unify ((t1,t2) :: rest)
	 | unify' ((DepTy (tyS1,_), tyS2) :: rest) =
	 	unify ((tyS1,tyS2) :: rest)
	 | unify' ((tyS1, DepTy (tyS2,_)) :: rest) =
	 	unify ((tyS1,tyS2) :: rest)
	...
	and unify [] = (print "Unify: empty!\n"; [])
	  | unify ((x,y)::t) = (print ("Unify: " ^ PrettyPrint.ppty x ^ " with " ^ 
							PrettyPrint.ppty y ^ "\n"); unify' ((x,y)::t))
